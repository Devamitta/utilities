...
class ReadOds:
    ...
    def save_csv(self):
        ...
        for sheet_name in self.df:
            ...
            # if sheet_name == "Roots":
            #     self.df[sheet_name].sort_values(by=['Root'], axis='index', ignore_index=True, inplace=True)
            #     filter = self.df[sheet_name]['Root'] != ""
            #     self.df[sheet_name] = self.df[sheet_name][filter]

            rows = self.df[sheet_name].shape[0]
            columns = self.df[sheet_name].shape[1]
            self.df[sheet_name].to_csv(f'{file_path}-{sheet_name.lower()}.csv', sep='\t', index=False, quoting=1)
            print(f" {file_path}-{sheet_name.lower()}.csv {rows} rows {columns} columns")
